...
def init_harvester_db():                                         
    ...
    if is_postgres:                                                  
        ...
        cursor.execute("ALTER TABLE leads ADD COLUMN IF NOT EXISTS ai_staged_image_url TEXT") # UPGRADE: AI Ready
    else:
        cursor.execute('''                                               
            CREATE TABLE IF NOT EXISTS leads (                               
        ...
        for col in ["property_type", "location", "contact_info", "created_at", "email", "phone", "source_url", "ai_staged_image_url"]:
...
def ingest_lead():                                               
    try:                                                             
        ...
        ai_staged_image = data.get('ai_staged_image_url') # UPGRADE: AI Intake
        ...
        if 'property_type' in data:
            ...
            if source_url:                                                   
                ...
                if existing_record:                                              
                    ...
                    cursor.execute(f"""                                              
                        UPDATE leads
                        SET price_ngn = {param}, email = {param}, phone = {param}, ai_staged_image_url = {param}, created_at = {param}
                        WHERE id = {param}                                       
                        """, (new_price, clean_email, phone, ai_staged_image, tstamp, old_id))                                                                                                                                  
                    conn.commit()                                                
                    cursor.close()                                               
                    conn.close()                                                 
            ...
            cursor.execute(f"""                                              
                INSERT INTO leads (property_type, location, price_ngn, email, phone, source_url, ai_staged_image_url, created_at)                              
                VALUES ({param}, {param}, {param}, {param}, {param}, {param}, {param}, {param})""",
                (prop_type, loc, new_price, clean_email, phone, source_url, ai_staged_image, tstamp))                                      
